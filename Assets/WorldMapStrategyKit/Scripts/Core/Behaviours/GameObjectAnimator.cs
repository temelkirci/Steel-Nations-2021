...
namespace WorldMapStrategyKit {
    ...
    public class GameObjectAnimator : MonoBehaviour, IExtendableAttribute {
        ...
        {
            shield.transform.localScale *= range;
        }
        public void CreateShield(GameObject shieldGO)
        {
            shield = Instantiate(shieldGO);

            map.AddMarker3DObject(shield, currentMap2DLocation);
            SetShieldRange(GetDivision().GetDivisionMaximumAttackRange());
        ...
        {
            if ((int)MY_UNIT_TYPE.DOCKYARD == type || (int)MY_UNIT_TYPE.COUNTRY_CAPITAL_BUILDING == type || (int)MY_UNIT_TYPE.REGION_CAPITAL_BUILDING == type || (int)MY_UNIT_TYPE.CITY_BUILDING == type)
            {
                return true;
            }
        ...
        public void CheckBuoyancyEffect() {
            if (enableBuoyancyEffect && terrainCapability != TERRAIN_CAPABILITY.OnlyGround && map.VGOBuoyancyAmplitude > 0 && map.lastKnownZoomLevel <= map.VGOBuoyancyMaxZoomLevel && isOnWater) {
                // Buoyancy effect
